...
exports.putUpdateUserProfile = async (req, res, next) => {
  try {
    ...
    await User.findByIdAndUpdate(req.session.user._id, userUpdateInfo);
...
exports.postIssueBook = async (req, res, next) => {
  if (req.session.user.violationFlag) {
    req.flash(
      "error",
  ...
  if (req.session.user.bookIssueInfo.length >= 5) {
...
exports.postRenewBook = async (req, res, next) => {
  try {
    const searchObj = {
      "user_id.id": req.session.user._id,
...
exports.postReturnBook = async (req, res, next) => {
  try {
    ...
    const pos = req.session.user.bookIssueInfo.indexOf(req.params.book_id);
    ...
    // removing issue
    const issue = await Issue.findOne({ "user_id.id": req.session.user._id });
    await issue.remove();

    // popping book issue info from user
    req.session.user.bookIssueInfo.splice(pos, 1);
